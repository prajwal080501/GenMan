function Features() {
  return (
    <div className="max-w-7xl mb-5  mx-auto rounded-lg p-3 mt-10 bg-gray-100">
      <div className="flex items-center justify-center">
        <p className="text-4xl font-extrabold">Features</p>
      </div>
      <div className="flex space-x-4 items-center justify-evenly">
        <div>
          <img
            src="https://icons.veryicon.com/png/o/business/work-circle/encrypted-3.png"
            className="w-52 h-52"
            alt=""
          />
          <p className="text-3xl text-center font-extrabold">Encrypted</p>
        </div>
        <div>
          <p className="text-3xl text-center font-extrabold">Authentication</p>
        </div>
        <div>
          <img
            src="https://ouch-cdn2.icons8.com/eo17CiVWxmJFs93DinSshdcHMjuhQ42Bb0JIHQlW2Xo/rs:fit:368:400/czM6Ly9pY29uczgu/b3VjaC1wcm9kLmFz/c2V0cy9wbmcvNzUw/LzNhMzI1YjRmLWYz/N2ItNDU5YS05YzFm/LWJlYjRlZTNhNmZk/OC5wbmc.png"
            className="w-32 h-32"
            alt=""
          />
          <p className="text-3xl text-center font-extrabold">Secure</p>
        </div>
      </div>
    </div>
  );
}

export default Features;
